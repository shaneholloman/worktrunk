...
**Gemini extension — `plugins/gemini/`.** Gemini CLI gets its own sibling payload dir (it can't share `plugins/worktrunk/`, whose `hooks/hooks.json` slot is Claude's). It holds `gemini-extension.json`, `hooks/hooks.json`, a `skills -> ../../skills` symlink, and a **bundled `hooks/wt.sh`** — byte-identical to `plugins/worktrunk/hooks/wt.sh`, drift-guarded by `test_plugin_layout_is_consolidated` — because `gemini extensions install` *copies* the extension dir, so a cross-dir reference to the shared shim would break in the copy. Two constraints follow from that copy behaviour and Gemini reading `gemini-extension.json` only at the source root: (1) **github-URL install is unsupported** (`gemini extensions install owner/repo` reads `<clone-root>/gemini-extension.json`, absent with the manifest under `plugins/gemini/`) — install via a local path until a `wt config plugins gemini install` command exists; (2) Gemini's `fs.cp` rewrites the relative `skills` symlink to an **absolute path**, so a local install works only while the source repo stays in place, and a git/URL install would lose the skills entirely (tempdir target deleted → silently empty). The robust fix — bundling copied SKILL.md trees at release time, drift-guarded like `wt.sh` — is tracked with the `wt config plugins gemini install` follow-up.
